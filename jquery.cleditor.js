...
(function($)
{
    ...
    $.cleditor = {
        ...
        defaultOptions:
        {
            ...
            bodyStyle: // style to assign to document body contained within the editor
                "margin:4px; font:14px sans-serif; cursor:text"
        },
    ...
    cleditor = function(area, options)
    {
        ...
        editor.options = options = $.extend(
        {}, $.cleditor.defaultOptions, options);
        ...
        $.each(options.controls.split(" "), function(idx, buttonName)
        {
            ...
            else
            {
                ...
                else if (button.image) map.backgroundImage =
                    imageUrl(button.image);
                if (button.stripIndex) map.backgroundPosition =
                    button.stripIndex * -24;
                $buttonDiv.css(map);
                ...
                // Create the popup
                if (button.popupName)
                    createPopup(button.popupName, options,
                        button.popupClass,
        ...
        if (!documentClickAssigned)
        {
            $(document).click(function(e)
            {
                ...
                var $target = $(e.target);
                if (!$target.add($target.parents()).is("." +
                        PROMPT_CLASS))
                    hidePopups();
            });
    ...
    {
        var $this = $(this);
        return handler ? $this.bind(BLURRED, handler) : $this.trigger(
            BLURRED);
    };
    ...
    {
        var $this = $(this);
        return handler ? $this.bind(CHANGE, handler) : $this.trigger(
            CHANGE);
    };
    ...
    {
        var $this = $(this);
        return handler ? $this.bind(FOCUSED, handler) : $this.trigger(
            FOCUSED);
    };
    ...
    function buttonClick(e)
    {
        ...
        else if (!sourceMode(editor))
        {
                        ...
                        .bind(CLICK, function()
                        {
                            ...
                            var $textarea = $popup.find("textarea"),
                                text = $textarea.val().replace(/\n/g,
                                    "<br />");
                            if (text !== "")
                                execCommand(editor, data.command, text,
                                    null, data.button);
            ...
            // All other buttons
            else if (!execCommand(editor, data.command, data.value, data.useCSS,
                    buttonDiv))
                return false;
    ...
    function popupClick(e)
    {
        ...
        // Execute the command
        if (data.command && !execCommand(editor, data.command, data.value,
        ...
        else
        {
            ...
            if (!success)
            {
                if ("cutcopypaste".indexOf(command) > -1)
                    ...
                    showMessage(editor,
                        (message ? message : "Error executing the " +
                            command + " command."),
                        button);
            }
        }
            ...
            return function() {
                ...
                if (ie || iege11)
                {
                    ...
                    $doc.bind(
                        ...
                        function(e)
                        {
                            ...
                            {
                                if (!editor.inactive && editor.range && editor.range
                                    .length > 1)
                                    editor.range.shift();
                ...
                $doc.click(hidePopups)
                    .keydown(function(e)
                    {
                        ...
                        if (ie && getSelection(editor).type == "Control" && e.keyCode ==
                            8)
                        {
    ...
    function refreshButtons(editor)
    {
        ...
        $.each(editor.$toolbar.find("." + BUTTON_CLASS), function(idx, elem)
        {
            ...
            var $elem = $(elem),
                button = $.cleditor.buttons[$.data(elem,
                    BUTTON_NAME)],
                command = button.command,
                enabled = true;
            ...
            else if (((inSourceMode || iOS) && button.name !==
                    "source") ||
            ...
            else if (command && command !== "print")
            {
                ...
                if ((!ie && !iege11) || command !== "inserthtml")
                {
                    try
                    {
                        enabled = queryObj.queryCommandEnabled(
                            command);
                    }
                    catch (err)
                    {
    ...
    function updateTextArea(editor, checkForChange)
    {
        ...
        var code = updateTextAreaCallback ? updateTextAreaCallback(html) :
            html;
